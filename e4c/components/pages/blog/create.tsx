...
const TextEditor: React.FC = () => {
  ...
  const handleFileChange = (event: any) => {
    ...
    if (file) {
      ...
      if (file.type.startsWith("image/")) {
        ...
        reader.onloadend = () => {
          setBlog({ ...blog, avatar: reader.result as string });
        };

        reader.readAsDataURL(file);
      } else {
        notifyError("Vui lòng chọn file là ảnh"); // Thông báo lỗi nếu không phải hình ảnh
      }
    }
  };
  ...
  const addBlog = async () => {
    ...
    try {
      const res = await axios.post(API_BLOG, {
        title: blog.title,
